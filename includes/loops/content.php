...
<?php if(have_posts()): while(have_posts()): the_post();?>
    <article role="article" id="post_<?php the_ID()?>">
        ...
        <section>
            ...
            <?php the_content( __( '&hellip; ' . __('Continue reading', 'hrm' ) . ' <i class="fa fa-arrow-right"></i>', 'hrm' ) ); ?>
        </section>
    </article>
<?php endwhile; ?>

<?php if ( function_exists('hrm_pagination') ) { hrm_pagination(); } else if ( is_paged() ) { ?>
  <ul class="pagination">
    <li class="older"><?php next_posts_link('<i class="fa fa-arrow-left"></i> ' . __('Previous', 'hrm')) ?></li>
    <li class="newer"><?php previous_posts_link(__('Next', 'hrm') . ' <i class="fa fa-arrow-right"></i>') ?></li>
  </ul>
<?php } ?>
