(function ($) {
    ...
    if ($('#toc_container').length > 0) { 
      $("#toc_container").addClass("callout");
		...
		// Click To Tweet https://gist.github.com/psteinweber/8369833
		/* NOT CONVIENCED ABOUT THIS YET
		var completeurl = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search;
		completeurl = encodeURIComponent(completeurl);
		var twshare = "https://twitter.com/share?url=" + completeurl;

		$("blockquote.hrm-tshare").each(function () {
			var $link = $("<a />"),
				linkText = "",
				url = twshare + "&text=" + $(this).text() + " @fridgebusiness";
			$link.attr("href", url);
		...
		*/
		...
		/* $(".hrm-share-button").on("click", function(e) {
			e.preventDefault();
